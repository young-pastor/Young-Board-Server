...
package com.zhisida.core.pojo.login;
...
public class SysLoginUser implements UserDetails, Serializable {
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        ArrayList<BoardAuthority> grantedAuthorities = CollectionUtil.newArrayList();
        if (ObjectUtil.isNotEmpty(roles)) {
            roles.forEach(dict -> {
                String roleName = dict.getStr(CommonConstant.NAME);
                BoardAuthority boardAuthority = new BoardAuthority(roleName);
                grantedAuthorities.add(boardAuthority);
            });
        }
